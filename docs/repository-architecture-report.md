...
| --- | --- | --- |
| `assets/shared.js` | storage helpers, timezone helpers, language and document-metadata wiring, footer rendering, shared labels and filter-control markup | `getInitialLanguage()`, `setDocumentLanguage()`, `readStorage()`, `renderSiteFooter()`, `renderFilterPill()`, `initLanguageDropdown()` |
